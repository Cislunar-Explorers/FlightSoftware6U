...
def output_CZML(
    trajDfpath, attDfPath, original_csv_timescale_in_seconds, startDate, endDate
):
    """
    ...
    timestep = (
        original_csv_timescale_in_seconds  # Sample rate of original csv in seconds
    )
    time = list(
        np.arange(0, len(trajDf.index) * original_csv_timescale_in_seconds, timestep)
    )
    ...
    for i in tqdm(range(len(time)), desc="Writing to CZML"):
        ...
        cam1string = [
            int(time[i]),
            float(camera1vector[0]),
            float(camera1vector[1]),
            float(camera1vector[2]),
        ]
        cam2string = [
            int(time[i]),
            float(camera2vector[0]),
            float(camera2vector[1]),
            float(camera2vector[2]),
        ]
        cam3string = [
            int(time[i]),
            float(camera3vector[0]),
            float(camera3vector[1]),
            float(camera3vector[2]),
        ]
...
if __name__ == "__main__":
    ...
    INITIAL_TRAJ_PATH = os.path.join(
        ...
        "OpNav",
        "data",
        "CislunarFullTraj_60secs",
        "trajectory",
        "1min_stk_active_sampled_traj.csv",
    )
    ...
    INITIAL_ATT_PATH = os.path.join(
        ...
        "CislunarFullTraj_60secs",
        "attitude",
        "discrete_att_with_maneuver_nutations.csv",
    )
