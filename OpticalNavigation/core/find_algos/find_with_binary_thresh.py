...
def get_earth_moon_coords(src):
    ...
    while overlaps and len(contours) > 0:
        ...
        overlaps = False
        if (
            (x2 > x and x2 < x + w and y2 > y and y2 < y + h)
            or (x2 + w2 > x and x2 + w2 < x + w and y2 > y and y2 < y + h)
            or (x2 + w2 > x and x2 + w2 < x + w and y2 + h2 > y and y2 + h2 < y + h)
            or (x2 > x and x2 < x + w and y2 + h2 > y and y2 + h2 < y + h)
        ):
            overlaps = True
...
def detect_circles(og, img):
    ...
    circles = cv2.HoughCircles(
        ...
        param1=100,
        param2=10,
        minRadius=int(sml.shape[0] / 8),
        maxRadius=sml.shape[0] * 2,
    )
