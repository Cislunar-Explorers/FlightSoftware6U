...
def main():
    ...
    cameras = {
        ...
        "C": Camera("C", width, height, fov_h, fov_v, Quaternion(axis=zhat, degrees=180 + 53)),
    }
    ...
    with open(os.path.join(SIM_DIR, "data/trajectory_sim_easy/observations.json"), "w") as f:
...
class Camera:
    def __init__(self, name, width, height, fov_h, fov_v, q_body2cam):
        ...
        self.gnomonic_scale = (
            ((self.width - 1) / 2) / np.tan(fov_h / 2) + ((self.height - 1) / 2) / np.tan(fov_v / 2)
        ) / 2
        ...
        self.stereographic_scale = (
            ((self.width - 1) / 2) / (2 * np.tan(fov_h / 4)) + ((self.height - 1) / 2) / (2 * np.tan(fov_v / 4))
        ) / 2
    ...
    for camera in cameras.values():
        ...
        for f in range(n_frames):
            ...
            if detection_dicts:
                ...
                cv2.imwrite(
                    os.path.join(SIM_DIR, "data/trajectory_sim_easy/images", filename_gn),
                    img,
                )
                ...
                cv2.imwrite(
                    os.path.join(SIM_DIR, "data/trajectory_sim_easy/images", filename_st),
                    img,
                )
    ...
    for row in range(camera.height):
        ...
        for b in range(len(obs_bodies)):
            ...
            if illuminator is None or obs_bodies[b] is illuminator:
                ...
                sun_pos_cam = q_world2cam.conjugate.rotate(illuminator.relpos_world).astype(sc.dtype)
                body_pos_cam = q_world2cam.conjugate.rotate(obs_bodies[b].relpos_world).astype(sc.dtype)
                radius = np.asarray(obs_bodies[b].body.radius, dtype=sc.dtype)
                img[row, ...] += colors_bgr[b] * is_illuminated(sun_pos_cam, body_pos_cam, radius, sc)[..., np.newaxis]
    return img
...
def render_stereographic(camera, obs_bodies, colors_bgr, illuminator):
    ...
    for b in range(len(obs_bodies)):
        ...
        if illuminator is None or obs_bodies[b] is illuminator:
            ...
            img += (
                colors_bgr[b]
                * (d < np.asarray(math.sin(obs_bodies[b].angular_radius) ** 2, dtype=d.dtype))[..., np.newaxis]
            )
        else:
            sun_pos_cam = camera.q_body2cam.conjugate.rotate(illuminator.position_body).astype(sc.dtype)
            body_pos_cam = camera.q_body2cam.conjugate.rotate(obs_bodies[b].position_body).astype(sc.dtype)
            radius = np.asarray(obs_bodies[b].body.radius, dtype=sc.dtype)
            img += colors_bgr[b] * is_illuminated(sun_pos_cam, body_pos_cam, radius, sc)[..., np.newaxis]
    return img
