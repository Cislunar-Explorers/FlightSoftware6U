...
def computeMeasurement(earthProperties, moonProperties, sunProperties, omega, dt):
    ...
    # Measurements
    z1 = np.linalg.norm(np.abs(earthProperties['center'][0] - moonProperties['center'][0]) + horiz_pixelsEM + vertPix[earthProperties['flag']][moonProperties['flag']]*zero_one)
    z2 = np.linalg.norm(np.abs(earthProperties['center'][0] - sunProperties['center'][0]) + horiz_pixelsES + vertPix[earthProperties['flag']][sunProperties['flag']]*zero_one)
    z3 = np.linalg.norm(np.abs(sunProperties['center'][0] - moonProperties['center'][0]) + horiz_pixelsMS + vertPix[moonProperties['flag']][sunProperties['flag']]*zero_one)
...
def cameraMeasurements(omega, dt):
        ...
        for i, file in enumerate(files):
            ...
            try:
                ...
                if len(earthProperties['index']) == 0:
                    loadProperties(copy.copy(true_img), 'earth', cam, i, earthProperties)
                # Find Moon
    ...
    atleastOneBodyNotFound = len(earthProperties['index']) == 0 or len(moonProperties['index']) == 0 or len(sunProperties['index']) == 0
    if len(earthProperties['index']) == 0:
