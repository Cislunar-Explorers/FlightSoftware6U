...
def extractMeasurements(circlesDf, measurementDir):
    ...
    while index < len(circleDict["Iteration"]):
        ...
        if circleDict["EX"][index] != "-":
            ...
            earths["pos"].append(
                [
                    float(circleDict["EX"][index]),
                    float(circleDict["EY"][index]),
                    float(circleDict["ER"][index]),
                ]
            )
        # Moon exists
        if circleDict["MX"][index] != "-":
            ...
            moons["pos"].append(
                [
                    float(circleDict["MX"][index]),
                    float(circleDict["MY"][index]),
                    float(circleDict["MR"][index]),
                ]
            )
        # Sun exists
        if circleDict["SX"][index] != "-":
            ...
            suns["pos"].append(
                [
                    float(circleDict["SX"][index]),
                    float(circleDict["SY"][index]),
                    float(circleDict["SR"][index]),
                ]
            )
        index += 1
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-c", "--circlecombined", help="path to the circlesCombined.csv file"
    )
