...
def __dynamics_model(state, dt, moonEph, sunEph, main_thrust_info):
    ...
    if main_thrust_info is not None:
        ...
        net_acceleration_thrust = (np.dot(Aq, TrajUKFConstants.ACCELERATION_DIR.data[:3].T.reshape(3,1)) * acc_mag).reshape(1,3)
...
def __findCovariances(xMean, zMean, propSigmas, sigmaMeasurements, centerWeight, otherWeight, alpha, beta, R):
    """
...
def __newEstimate(xMean:np.ndarray, zMean:np.ndarray, Pxx:np.ndarray, 
                Pxz:np.ndarray, Pzz:np.ndarray, measurements:np.ndarray, 
                R:np.ndarray, dynamicsOnly=False) -> TrajectoryEstimateOutput:
    """
...
def runTrajUKF(moonEph: EphemerisVector, sunEph: EphemerisVector, measurements:CameraMeasurementVector, 
    ...
    if main_thrust_info is not None:
        main_thrust_info.get_kick_orientation().data = main_thrust_info.get_kick_orientation().data.reshape(4,1)
    ...
    for j in range(__length(sigmas)):
        propSigmas[:,j] = __dynamics_model(sigmas[:,j]+noise[:,j], dt, moonEph.data, sunEph.data, main_thrust_info=main_thrust_info)
    ...
    for j in range(__length(sigmas)):
        sigmaMeasurements[:,j] = __measModel(propSigmas[:,j] + np.random.multivariate_normal(np.zeros((6,)),Const.R).T, moonEph.data, sunEph.data, const)
