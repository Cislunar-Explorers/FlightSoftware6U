...
def sixhours_direct_test(visual_analysis, state_error, part_start, part_end, timestep):
    ...
    d_camMeas, d_moonEph, d_sunEph, d_traj, totalIntegrationTime = get6HoursBatch(
        part_start,
        part_end,
        part_start,
        timestep,
    ...
    for t in tqdm(range(len(d_traj["x"])), desc="Trajectory Completion"):
        ...
        traj_out = runTrajUKF(
            ...
            CameraMeasurementVector(
                meas[0, 0], meas[1, 0], meas[2, 0], meas[3, 0], meas[4, 0], meas[5, 0]
            ),
...
def sixhours_db_test(mocker, state_error, part_start, part_end, timestep):
    ...
    for t in tqdm(range(len(d_traj["x"])), desc="Preparing test dbs"):
        ...
        measurements.append(
            OpNavCameraMeasurementModel.from_tuples(
                ...
                time=new_time,
            )  # camera measurement taken now, uses state
        )
        entries.extend(
            [
        ...
        session.bulk_save_objects(
            [
                ...
                OpNavGyroMeasurementModel.from_tuples(
                    measurement=(0.0, 0.0, 0.0), time=curr_time
                ),  # dummy gyro measurement 1
                OpNavGyroMeasurementModel.from_tuples(
                    measurement=(0.0, 0.0, 0.0), time=curr_time + timedelta(0, 1)
                ),  # dummy gyro measurement 2
            ]
        )
        session.commit()
        measurements.pop(0)
    ...
    mocker.patch(
        "core.opnav.__process_propulsion_events",
        side_effect=process_propulsion_events_mock,
    )

    def run_attitude_ukf_mock(*args, **kwargs):
        return AttitudeEstimateOutput(
            new_state=AttitudeStateVector(0, 0, 0, 0, 0, 0),
    ...
    status = opnav.start(
        ...
        gyro_count=2,
        camera_params=MatlabTestCameraParameters,
    )
    # verification
    ...
    assert (
        len(entries) == expected_new_entries_len + 1
    ), f"Expected {expected_new_entries_len + 1} entries in trajectory db, got {len(entries)}"
    # start() should succeed
    assert (
        status == OPNAV_EXIT_STATUS.SUCCESS
    ), f"Expected start() to succeed, got status {status}"
    ...
    entry = (
        ...
        .filter(OpNavTrajectoryStateModel.time_retrieved == timestamps[-1])
        .first()
    )  # search for state estimate made at time of measurement
    true_state = np.array(
        [
            d_traj["x"][i],
            d_traj["y"][i],
    ...
    print(f"Position error: {posError}\nVelocity error: {velError}")
    assert (
        posError <= POS_ERROR_6HOURS
    ), f"Position error is too large. Expected {POS_ERROR_6HOURS} Actual {posError}"
    assert (
        velError <= VEL_ERROR_6HOURS
    ), f"Velocity error is too large. Expected {VEL_ERROR_6HOURS} Actual {velError}"
    progress_bar.close()
...
class TestZeroStartingNoiseTimestep:
    ...
    def test_interval1_db(self, mocker):
        sixhours_db_test(mocker, ZERO_STARTING_NOISE, 0, 360, 1.0)

    def test_interval1_direct(self, mocker):
        sixhours_direct_test(False, ZERO_STARTING_NOISE, 0, 360, 1.0)
    ...
    def test_interval5_db(self, mocker):
        sixhours_db_test(mocker, ZERO_STARTING_NOISE, 0, 360, 2.0)

    def test_interval5_direct(self, mocker):
        sixhours_direct_test(False, ZERO_STARTING_NOISE, 0, 360, 2.0)
...
class TestSmallStartingNoiseTimestep:
    ...
    def test_interval1_db(self, mocker):
        sixhours_db_test(mocker, SMALL_STARTING_NOISE, 0, 360, 1.0)

    def test_interval1_direct(self, mocker):
        sixhours_direct_test(False, SMALL_STARTING_NOISE, 0, 360, 1.0)
    ...
    def test_interval5_db(self, mocker):
        sixhours_db_test(mocker, SMALL_STARTING_NOISE, 0, 360, 2.0)

    def test_interval5_direct(self, mocker):
        sixhours_direct_test(False, SMALL_STARTING_NOISE, 0, 360, 2.0)
...
class TestLargeStartingNoiseTimestep:
    ...
    def test_interval1_db(self, mocker):
        sixhours_db_test(mocker, LARGE_STARTING_NOISE, 0, 360, 1.0)

    def test_interval1_direct(self, mocker):
        sixhours_direct_test(False, LARGE_STARTING_NOISE, 0, 360, 1.0)
    ...
    def test_interval4(self, mocker):
        sixhours_direct_test(
            False, LARGE_STARTING_NOISE, 1100, 1400, 1
        )  # TODO: Volatile test: depends on random starting noise

    def test_interval5_db(self, mocker):
        sixhours_db_test(mocker, LARGE_STARTING_NOISE, 0, 360, 2.0)

    def test_interval5_direct(self, mocker):
        sixhours_direct_test(False, LARGE_STARTING_NOISE, 0, 360, 2.0)
    ...
    def test_interval8(self, mocker):
        sixhours_direct_test(
            False, LARGE_STARTING_NOISE, 1500, 1800, 2
        )  # TODO: Volatile test: depends on random starting noise
