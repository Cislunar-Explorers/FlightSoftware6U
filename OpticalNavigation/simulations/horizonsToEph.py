...
def extractVariable(line, re, varName, varForm, lineNum):
    ...
    v = re.search(line)
    if v is None:
        print(f'{varName} not found. Please check if horizons file contains {varName} coordinates in the form {varForm} @ line {lineNum}')
        return None
...
if __name__ == "__main__":
    ...
    for i in range(0, len(dataLines), 3): 
        ...
        if time is None:
            print(f'Time not found. Please check if horizons file contains time in approprate format @ line {i + start + 0 + 1}')
            exit()
        ...
        if x is None or y is None or z is None or vx is None or vy is None or vz is None: 
            exit()
        d['time'].append(time)
        d['x'].append(x)
        d['y'].append(y)
