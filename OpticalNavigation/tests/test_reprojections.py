...
class TestReprojections(unittest.TestCase):
    ...
    # and stereographic images as filenames ending in "_st.png"
    gnomonicList = glob.glob("images/*_gn.png")
    stereoList = glob.glob("images/*_st.png")
    ...
    for gnName in gnomonicList:
        ...
        # Determine which camera is used based on whether filename contains "camA", "camB", or "camC"
        if "camA" in gnName:
            u_cam = np.array(
        ...
        # Uncomment the following line in order to save the remapped image with filename
        # "remapped_" + gnName to folder "out"
        ...
        for i, c in enumerate(contoursOut):
            ...
            for j, c2 in enumerate(contoursTgt):
                ...
                # They should be close to the same size; if they aren't, they are likely not
                # the same object.
                ...
                print(
                    "Image ", gnName, " contour ", i, " remapped with difference ", diff
                )

                minW = 180

                # If the image width is less than 256, resize all three images to have width of 256
                # and height determined by the aspect ratio of the original image.
                if w < minW:
                    outc = cv2.resize(
                        outc, (minW, int(minW * h / w)), interpolation=cv2.INTER_AREA
                    )
                    tgtc = cv2.resize(
                        tgtc, (minW, int(minW * h / w)), interpolation=cv2.INTER_AREA
                    )
                    cmp = cv2.resize(
                        cmp, (minW, int(minW * h / w)), interpolation=cv2.INTER_AREA
                    )
                ...
                # Uncomment the following line in order to save the composite image with filename
                # "composite_" + outputString to folder "out".
