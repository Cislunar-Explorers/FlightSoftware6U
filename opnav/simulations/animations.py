...
def testArrow():
    ...
    a = Arrow3D(
        [0, 1], [0, 0], [0, 0], mutation_scale=20, lw=3, arrowstyle="-|>", color="r"
    )
    ax.add_artist(a)
...
class LiveTrajectoryPlot:
    ...
    def renderUKF(self, delay=0.001, text=""):
        ...
        self.ax.plot(
            self.estimated_traj["x"],
            self.estimated_traj["y"],
            self.estimated_traj["z"],
            color="blue",
...
class Live2DPlot:
    ...
    def updateGraph(self, id, x, y):
        ...
        traceLim = self.traceLimits[
            id
        ]  # Grab last N elements, which removes older portions of the trajectory
        if traceLim is None:
            traceLim = len(
                self.graphs[id]["x"]
            )  # Do not remove any portion of the path
    ...
    def render(self, delay=0.001, text=""):
        ...
        for i in range(len(self.arrows)):
            ...
            a = Arrow3D(
                ...
                color=self.settings[i]["color"],
                alpha=self.settings[i]["alpha"],
            )
            ...
            self.ax.plot(
                ...
                color=self.settings[i]["color"],
                alpha=0.5,
                markersize=self.settings[i]["size"],
                marker=f"${name}$",
            )
...
class LiveMultipleTrajectoryPlot:
    ...
    def addTrajectory(
        self, color, label, alpha, ls, traceLim, blobsize, blobshape, bloblabel
    ):
        """
    ...
    def updateTraj(self, i, x, y, z):
        ...
        self.trajectories[i]["x"] = self.trajectories[i]["x"][
            -self.trajectories[i]["trace_lim"] :
        ]
        self.trajectories[i]["y"] = self.trajectories[i]["y"][
            -self.trajectories[i]["trace_lim"] :
        ]
        self.trajectories[i]["z"] = self.trajectories[i]["z"][
            -self.trajectories[i]["trace_lim"] :
        ]
    ...
    def updateAtt(self, i, start_pos, new_vector):
        ...
        scale = max(
            (scaleX / 10 + scaleY / 10 + scaleZ / 10) / 3,
            (scaleX / 5 + scaleY / 5 + scaleZ / 5) / 3,
        )
    ...
    def addTrackingArrow(
        self, mutation_scale=20, lw=2, ls="-", style="-|>", color="r", name="", size=10
    ):
        i = len(self.trackingArrows)
        self.trackingArrows.append(
            {
                "x": [None, None],
                "y": [None, None],
                "z": [None, None],
