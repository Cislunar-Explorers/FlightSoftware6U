...
import argparse
from simulations.getManeuvers import (
    extractCheckpoints,
    getMissionTimeline,
...
if __name__ == "__main__":
    ...
    INITIAL_ATT_PATH = os.path.join(
        args["opnavdataset"], "attitude", "discrete_att_with_maneuver_nutations.csv"
    )
    ...
    moonEphIndex = liveTraj.addTrajectory(
        color="red",
        label="moon 1 min eph",
        alpha=0.5,
        ls=":",
        traceLim=100,
        blobsize=5,
    ...
    satTrajIndex = liveTraj.addTrajectory(
        color="green",
        label="sat 1 min traj",
        alpha=0.5,
        ls=":",
        traceLim=100000000,
        blobsize=10,
        blobshape="$L$",
        bloblabel="CisLunar Explorer",
    )
    earthBlobIndex = liveTraj.addTrajectory(
        color="blue",
        label=None,
        alpha=0.75,
        ls=None,
    ...
    xVNCIndex = liveTraj.addTrackingArrow(
        style="-|>", color="orange", lw=1, ls="-", name="X_{VNC}", size=30
    )
    yVNCIndex = liveTraj.addTrackingArrow(
        style="-|>", color="orange", lw=1, ls="-", name="Y_{VNC}", size=30
    )
    zVNCIndex = liveTraj.addTrackingArrow(
        style="-|>", color="orange", lw=1, ls="-", name="Z_{VNC}", size=30
    )
    velAxisIndex = liveTraj.addTrackingArrow(
        style="-|>", color="black", lw=1, ls="dashed", name="V_B", size=20
    )

    xBodyIndex = liveTraj.addTrackingArrow(
        style="-|>", color="lime", lw=1, ls="-", name="X_B", size=20
    )
    yBodyIndex = liveTraj.addTrackingArrow(
        style="-|>", color="lime", lw=1, ls="-", name="Y_B", size=20
    )
    zBodyIndex = liveTraj.addTrackingArrow(
        style="-|>", color="lime", lw=1, ls="-", name="Z_B", size=20
    )

    maneuverArrowIndexMap = []  # checkpoint -> arrow index
    for i in range(len(maneuversDict["startTime"])):
        start_index = liveTraj.addTrajectory(
            color="magenta",
            label=None,
            alpha=0.50,
            ls="-",
    ...
    for index, timestamp in enumerate(missionTimeline):
        if index % (simMultiplier) == 0:  # sample at hour interval
            ...
            liveTraj.updateTraj(
                moonEphIndex, moonDf["x"][index], moonDf["y"][index], moonDf["z"][index]
            )
            # liveTraj.updateTraj(1, sunDf['x'][index], sunDf['y'][index], sunDf['z'][index])
            liveTraj.updateTraj(
                satTrajIndex, trajDf["x"][index], trajDf["y"][index], trajDf["z"][index]
            )
            start_pos = np.array(
                [trajDf["x"][index], trajDf["y"][index], trajDf["z"][index]]
            )
            # Velocity
            new_vector = np.array(
                [trajDf["vx"][index], trajDf["vy"][index], trajDf["vz"][index]]
            )
            new_vector /= np.linalg.norm(new_vector)
            liveTraj.updateAtt(velAxisIndex, start_pos, new_vector)

            # Maneuver
            if currentManeuver >= 0 and currentManeuver < len(
                maneuversDict["startTime"]
            ):
                isManeuver = (
                ...
                if isManeuver:
                    liveTraj.updateTraj(
                        maneuverArrowIndexMap[currentManeuver],
                        start_pos[0],
                        start_pos[1],
                        start_pos[2],
                    )
            ...
            liveTraj.renderUKF(
                text="{}%\n{}\nx{}".format(
                    round(index / len(missionTimeline) * 100, 2),
                    currentDateTime,
                    simMultiplier,
                ),
                delay=0.001,
