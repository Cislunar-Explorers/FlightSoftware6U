...
class Power:
    ...
    def set_output(self, byte):
        logging.debug("Setting outputs to %s", byte)
    ...
    # Not tested
    def get_heater(self):
        logging.debug("Running get_heater")
    ...
    def reset_counters(self):
        logging.debug("Resetting boot and WDT counters")
        self.write(CMD_RESET_COUNTERS, [0x42])

    # resets (kicks) dedicated WDT.
    # Not tested
    def reset_wdt(self):
        logging.debug("Resetting WDT")
    ...
    def config_get(self) -> ps.eps_config_t:
        logging.debug("Getting current config")
    ...
    def config_set(self, struct: ps.eps_config_t):
        logging.debug("Setting current config")
    ...
    def solenoid(self, spike, hold):
        ...
        logging.warning("DEPRECATED FUNCTION Power().solenoid")
        logging.debug(
