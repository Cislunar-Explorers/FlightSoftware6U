...
#     cv2.imshow("Grayscale - Moon", gray)
#     cv2.waitKey(0)
#     # scale this up first
...
#     upper_black = np.array([255], dtype = "uint16")
#     black_mask = cv2.inRange(gray, lower_black, upper_black)
#     cv2.imshow("Contrast Boost - Moon", black_mask)
#     cv2.waitKey(0)
#     scale = 1
#     (c, r) = np.shape(black_mask)
#     rows = int(np.round(r * scale))
...
#     cv2.imshow("Auto Canny - Moon", auto_canny)
#     cv2.waitKey(0)
...
#     cv2.imshow("Median Blur - Moon", original_median_blurred)
#     cv2.waitKey(0)
...
#     a = int(np.round(a * 0.5))
#     b = int(np.round(b * 0.5))
