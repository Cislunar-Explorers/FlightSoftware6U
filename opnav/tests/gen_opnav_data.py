...
def propagateSpacecraft(X, t, kickTime, duration=10):
    ...
    inner_sc = (
        np.dot(crs(omega_sc), np.dot(SPACECRAFT_I_B, omega_sc))
        - DAMPER_C * omega_d
        + int(0.5 * (np.sign(t - kickTime) + 1)) * TORQUE_THRUSTER
        - int(0.5 * (np.sign(t - (kickTime + duration)) + 1)) * TORQUE_THRUSTER
    )
    omega_sc_dot = np.dot(-1.0 * pinv(SPACECRAFT_I_B), inner_sc)

    right_d = (
        np.dot(crs(omega_sc), np.dot(DAMPER_I_D, omega_d + omega_sc))
        + DAMPER_C * omega_d
    )
    inner_d = np.dot(DAMPER_I_D, omega_sc_dot) + right_d
    omega_d_dot = np.dot(-1.0 * pinv(DAMPER_I_D), inner_d)
...
def generateSyntheticData(
    ...
    gyroSigma,
    gyroNoiseSigma,
    integrationTimeStep=INTEGRATION_TIMESTEP,
):
    """
    ...
    angular_momentum_history = plotSpacecraft(
        omega_init, totalIntegrationTime, integrationTimeStep, kickTime, duration
    )
    ...
    quaternion_history = obtainAllQuatsfromAngVel(
        q0.flatten(),
        totalIntegrationTime,
        integrationTimeStep,
    ...
    # Propagate bias
    bias_history = goBias(
        totalIntegrationTime, gyro_sample_rate, bias_init, gyroSigma, gyroNoiseSigma
    )
...
def plotResults(
    results: List[AttitudeEstimateOutput], q1, q2, q3, q4, biasx, biasy, biasz, timeline
):
    """
    ...
    for i in range(len(qhist)):  # quat
        ...
        deltaq4 = errquat[3][0]
        attitude_error.extend([2.0 * np.arccos(deltaq4) * (180.0 / np.pi)])
