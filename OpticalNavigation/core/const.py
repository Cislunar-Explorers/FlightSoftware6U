...
class CameraParameters:
    ...
    def __init__(self, hFov: float, vFov: float, hPix: int, vPix: int,
        ...
        c1rz = np.array([math.cos(c1az), -1 * math.sin(c1az), 0, math.sin(c1az), math.cos(c1az), 0, 0, 0, 1]).reshape(3,
                                                                                                                      3)
        c1ry = np.array([math.cos(c1ay), 0, math.sin(c1ay), 0, 1, 0, -1 * math.sin(c1ay), 0, math.cos(c1ay)]).reshape(3,
                                                                                                                      3)

        c2rz = np.array([math.cos(c2az), -1 * math.sin(c2az), 0, math.sin(c2az), math.cos(c2az), 0, 0, 0, 1]).reshape(3,
                                                                                                                      3)
        c2ry = np.array([math.cos(c2ay), 0, math.sin(c2ay), 0, 1, 0, -1 * math.sin(c2ay), 0, math.cos(c2ay)]).reshape(3,
                                                                                                                      3)

        c3rz = np.array([math.cos(c2az), -1 * math.sin(c2az), 0, math.sin(c2az), math.cos(c2az), 0, 0, 0, 1]).reshape(3,
                                                                                                                      3)
...
class GyroVars:
    ...
    def get_Q_matrix(self):
        return np.array([[self.gyro_noise_sigma ** 2. - (1. / 6.) * self.gyro_sigma ** 2. * self.gyro_sample_rate ** 2.,
                          0., 0., 0., 0., 0.],
                         [0.,
                          self.gyro_noise_sigma ** 2. - (1. / 6.) * self.gyro_sigma ** 2. * self.gyro_sample_rate ** 2.,
                          0., 0., 0., 0.],
                         [0., 0.,
                          self.gyro_noise_sigma ** 2. - (1. / 6.) * self.gyro_sigma ** 2. * self.gyro_sample_rate ** 2.,
                          0., 0., 0.],
