...
def formA(q):
    ...
    return np.array(
        [
            [
                1.0 - 2.0 * q2 ** 2.0 - 2.0 * q3 ** 2.0,
                2.0 * q1 * q2 - 2.0 * q3 * q4,
                2.0 * q1 * q3 + 2.0 * q2 * q4,
            ],
            [
                2.0 * q1 * q2 + 2.0 * q3 * q4,
                1.0 - 2.0 * q1 ** 2.0 - 2.0 * q3 ** 2.0,
                2.0 * q2 * q3 - 2.0 * q1 * q4,
            ],
            [
                2.0 * q1 * q3 - 2.0 * q2 * q4,
                2.0 * q1 * q4 + 2.0 * q2 * q3,
                1.0 - 2.0 * q1 ** 2.0 - 2.0 * q2 ** 2.0,
            ],
        ]
    )
...
def getGyroMeasurement(j, gyro_noise_sigma, ws, bs):
    ...
    return (
        (ws[j].data.reshape(3, 1))
        + (bs.reshape(3, 1))
        + np.random.randn(3, 1) * gyro_noise_sigma
    )
...
def computePsi(j, biasEst, gyro_sigma, sample_rate, ws, bs):
    ...
    return (
        np.sin(0.5 * sample_rate * np.linalg.norm(omegaplus))
        / np.linalg.norm(omegaplus)
    ) * omegaplus
...
def computeBigOmega(j, biasEst, gyro_sigma, sample_rate, ws, bs):
    ...
    oneone = np.cos(
        0.5 * sample_rate * np.linalg.norm(updateOmega(j, biasEst, gyro_sigma, ws, bs))
    )
    psicrs = crs(psi)
    lowerright = np.array(
        [
...
def generateSigmas(xhatkp, Phatkp, Q):
    ...
    sigpoints[0] = np.array(
        [[0.0, 0.0, 0.0, xhatkp[3][0], xhatkp[4][0], xhatkp[5][0]]]
    ).T
...
def makeErrorQuaternion(sigmas):
    ...
    for i in sigmas:
        ...
        deltaq4p = (
            -_a * np.linalg.norm(deltap) ** 2.0
            + _f * np.sqrt(_f ** 2.0 + (1 - _a ** 2.0) * np.linalg.norm(deltap) ** 2.0)
        ) / (_f ** 2.0 + np.linalg.norm(deltap) ** 2.0)
        deltaqhatkp = (1.0 / _f) * (_a + deltaq4p) * deltap
...
def quaternionComposition(firstq, secondq):
    ...
    mat = np.array(
        [[q4, q3, -q2, q1], [-q3, q4, q1, q2], [q2, -q1, q4, q3], [-q1, -q2, -q3, q4]]
    )
...
def propagateQuaternion(
    ...
    for i in range(len(perturbed_quat_list)):
        ...
        for j in enumerate(time_segment):
...
def quaternionInv(quat):
    return np.array(
        [[-1 * quat[0][0]], [-1 * quat[1][0]], [-1 * quat[2][0]], [1 * quat[3][0]]]
    )
...
def recoverPropSigma(quat_errors, old_sigmas):
    ...
    for i in range(len(quat_errors)):
        deltaqhat = np.array(
            [[quat_errors[i][0][0]], [quat_errors[i][1][0]], [quat_errors[i][2][0]]]
        )
        deltaq4 = quat_errors[i][3][0]
        top = (_f * deltaqhat) / (_a + deltaq4)
        bottom = np.array(
            [[old_sigmas[i][3][0]], [old_sigmas[i][4][0]], [old_sigmas[i][5][0]]]
        )
...
def predictedMean(newsigs):
    xmean = (1.0 / (NX + LAM)) * (LAM * newsigs[0])
    for i in newsigs[1:]:
        xmean += (1.0 / (2.0 * (NX + LAM))) * i
    return xmean


def predictedCov(newsigs, pred_mean, Q):
    P = ((LAM / (NX + LAM))) * np.dot(
        (newsigs[0] - pred_mean), (newsigs[0] - pred_mean).T
    )
    for i in newsigs[1:]:
        P += (1.0 / (2 * (NX + LAM))) * np.dot((i - pred_mean), (i - pred_mean).T)
    return P + Q
...
def meanMeasurement(hlist):
    zmean = (1.0 / (NX + LAM)) * (LAM * hlist[0])
    for i in hlist[1:]:
        zmean += (1.0 / (2 * (NX + LAM))) * i
    firstden = np.sqrt(zmean[0][0] ** 2.0 + zmean[1][0] ** 2.0 + zmean[2][0] ** 2.0)
    secondden = np.sqrt(zmean[3][0] ** 2.0 + zmean[4][0] ** 2.0 + zmean[5][0] ** 2.0)
    thirdden = np.sqrt(zmean[6][0] ** 2.0 + zmean[7][0] ** 2.0 + zmean[8][0] ** 2.0)
...
def Pzz(hlist, zmean, R):
    ...
    for i in hlist[1:]:
        Pzz += (1.0 / (2 * (NX + LAM))) * np.dot(i - zmean, (i - zmean).T)
    return Pzz + R
...
def Pxz(siglist, xmean, hlist, zmean):
    ...
    for i in range(1, len(siglist)):
        Pxzmat += (1.0 / (2 * (NX + LAM))) * np.dot(
            siglist[i] - xmean, (hlist[i] - zmean).T
        )
    return Pxzmat
...
def resetSigmaSeed(xhatkp):
    return np.array([[0.0, 0.0, 0.0, xhatkp[3][0], xhatkp[4][0], xhatkp[5][0]]]).T
...
def UKFSingle(
    ...
    # satPos, moonPos, sunPos = estimatedSatState[i:i+gyroSampleCount,:], moonEph[i:i+gyroSampleCount,:],
    #   sunEph[i:i+gyroSampleCount,:]
    # earthVec, moonVec, sunVec = np.zeros((gyroSampleCount, 3)), np.zeros((gyroSampleCount, 3)),
    #   np.zeros((gyroSampleCount, 3))
    # for g_sample in range(gyroSampleCount):
    # earthVec[i+g_sample] = (-1*satPos[i+g_sample])/np.linalg.norm(satPos[i+g_sample])
    # moonVec[i+g_sample] =
    #   (moonPos[i+g_sample] - satPos[i+g_sample])/np.linalg.norm(moonPos[i+g_sample] - satPos[i+g_sample])
    # sunVec[i+g_sample] =
    #   (sunPos[i+g_sample] - satPos[i+g_sample])/np.linalg.norm(sunPos[i+g_sample] - satPos[i+g_sample])
    ...
    prop_quats = propagateQuaternion(
        pert_quats,
        sigma_points,
        timeline,
        gyro_sigma,
...
def runAttitudeUKF(
    ...
    gyroVars: GyroVars,
    P0: CovarianceMatrix,
    x0: AttitudeStateVector,
    quat: QuaternionVector,
    omegas: List[GyroMeasurementVector],
    satState,
    moonEph: np.ndarray,
    sunEph: np.ndarray,
    timeline: List[float],
