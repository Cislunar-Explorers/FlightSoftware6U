...
class TestReprojections(unittest.TestCase):
    ...
    def get_ucam(self, name: str) -> np.array:
        ...
        cam_dict = {
            ...
            "cam3": np.array([-3.9931775502364646, -3.0090751157602416, 0.0], dtype=np.float32) / 5,
        }
    ...
    def reproj(self, src: np.ndarray, gnName: str) -> tuple([np.ndarray, tiled_remap.BoundingBox, str]):
    ...
    def get_images(self, path=None) -> tuple([list, list]):
        ...
        gn_path = os.path.join(
            (os.path.join(DATA_DIR, "traj-case1c_sim_no_outline") if path is None else path),
            "images/*_gn.png",
        )
    ...
    def reproj_test(self, write_remapped: bool, write_composite: bool, path=None) -> None:
        ...
        for idx, gnName in enumerate(gnomonicList):
            ...
            contoursOut, _ = cv2.findContours(threshOut, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            ...
            contoursTgt, _ = cv2.findContours(threshTgt, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
