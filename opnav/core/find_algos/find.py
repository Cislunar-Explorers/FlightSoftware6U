...
def findEarth(image):
    ...
    black_mask = np.clip(
        (gray_blurred + gray_blurred / 255 * 200).astype("uint8"), a_min=0, a_max=255
    )
...
def findMoon(image):
    ...
    # Increase brightness
    black_mask = np.clip(
        (gray_blurred + gray_blurred / 255 * 10).astype("uint8"), a_min=0, a_max=255
    )
...
def findHough(true_image, visualize=False):
    ...
    (origH, origW, _) = true_image.shape
    ...
    if circles is not None:
        ...
        if visualize:
            for _ in range(0, 1):
                cv2.circle(
                    original_with_circles,
                    (sunCircle[0], sunCircle[1]),
                    sunCircle[2],
                    (0, 255, 255),
                    2,
                )
    ...
    if circles is not None:
        ...
        if (
            earthCircle is not None
            and math.sqrt(
                (moonCircle[0] - earthCircle[0]) ** 2
                + (moonCircle[1] - earthCircle[1]) ** 2
            )
            < moonCircle[2] + earthCircle[2]
        ):
            searchSpace = [
                int((moonCircle[0] + earthCircle[0]) / 2),
                int((moonCircle[1] + earthCircle[1]) / 2),
                int(max(moonCircle[2], earthCircle[2])),
            ]
            ...
            if moonCircle[2] < earthCircle[2] or contains_blue(
                orig_copy[top:bottom, left:right]
            ):
    ...
    if visualize:
        ...
        if moonCircle is not None:
            cv2.circle(
                original_with_circles,
                (moonCircle[0], moonCircle[1]),
                moonCircle[2],
                (200, 200, 200),
                2,
            )
            cv2.circle(
                original_with_circles, (moonCircle[0], moonCircle[1]), 2, (0, 0, 255), 1
            )
...
def find(img) -> ImageDetectionCircles:
    ...
    sx, sy, sz, sdia = 0.0, 0.0, 0.0, 0.0  # Sun
    mx, my, mz, mdia = 0.0, 0.0, 0.0, 0.0  # Moon
    ex, ey, ez, edia = 0.0, 0.0, 0.0, 0.0  # Earth
