import os
from fsw.utils import parameter_utils, parameters
import json
import unittest
...
class ParametersTestCase(unittest.TestCase):
    def setUp(self) -> None:
        print(os.getcwd())
        if (
            "FlightSoftware/FlightSoftware/" not in os.getcwd()
        ):  # checks if running pytest
    ...
    def test_parameter_consistency(self):
        """Tests that the parameters defined in parameters.json are consistent with those defined in parameters.py"""

        py_list = parameter_utils.get_parameter_list()
        json_list = parameter_utils.get_parameter_list(
            hard=True, filename=self.filepath
        )
    ...
    def test_parameter_set(self):
        ...
        parameter_utils.set_parameter(
            param_name, new_parm_value, True, filename=self.filepath
        )
